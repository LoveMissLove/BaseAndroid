...
public final class CrashActivity extends BaseActivity {
    ...
    protected void initData() {
        ...
        builder.append("\n应用版本：\t").append(APKVersionCodeUtils.getVersionName(this))
                .append("\n版本代码：\t").append(APKVersionCodeUtils.getVersionCode(this));
        ...
        try {
            ...
            if (permissions.contains(PermissionConstant.READ_EXTERNAL_STORAGE) || permissions.contains(PermissionConstant.WRITE_EXTERNAL_STORAGE)) {
                builder.append("\n存储权限(读)：\t").append(PermissionUtil.showIsPermissionsGranted(this,PermissionConstant.READ_EXTERNAL_STORAGE) ? "已获得" : "未获得");
                builder.append("\n存储权限(写)：\t").append(PermissionUtil.showIsPermissionsGranted(this,PermissionConstant.WRITE_EXTERNAL_STORAGE) ? "已获得" : "未获得");
            }

            if (permissions.contains(PermissionConstant.ACCESS_FINE_LOCATION) ) {
                builder.append("\n定位权限：\t");
                builder.append("\n定位权限：\t").append(PermissionUtil.showIsPermissionsGranted(this,PermissionConstant.ACCESS_FINE_LOCATION) ? "已获得" : "未获得");

            }

            if (permissions.contains(PermissionConstant.CAMERA)) {
                builder.append("\n相机权限：\t").append(PermissionUtil.showIsPermissionsGranted(this,PermissionConstant.CAMERA)  ? "已获得" : "未获得");
            }

            if (permissions.contains(PermissionConstant.RECORD_AUDIO)) {
                builder.append("\n录音权限：\t").append(PermissionUtil.showIsPermissionsGranted(this,PermissionConstant.RECORD_AUDIO) ? "已获得" : "未获得");
            }

            if (permissions.contains(PermissionConstant.SYSTEM_ALERT_WINDOW)) {
                builder.append("\n悬浮窗权限：\t").append(PermissionUtil.showIsPermissionsGranted(this,PermissionConstant.SYSTEM_ALERT_WINDOW)  ? "已获得" : "未获得");
            }

            if (permissions.contains(PermissionConstant.REQUEST_INSTALL_PACKAGES)) {
                builder.append("\n安装包权限：\t").append(PermissionUtil.showIsPermissionsGranted(this,PermissionConstant.REQUEST_INSTALL_PACKAGES)  ? "已获得" : "未获得");
            }
